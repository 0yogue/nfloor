...
export function SalesFunnel({ metrics, is_loading }: SalesFunnelProps) {
  const stages: FunnelStage[] = [
    { label: "Leads", count: metrics.lead_count, color: "text-blue-600", bg_color: "bg-blue-500" },
    ...
    { label: "Vendas", count: metrics.sold_count, color: "text-green-600", bg_color: "bg-green-500" },
  ];
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          {total} leads no período • Taxa de conversão geral: {metrics.lead_count > 0 ? Math.round((metrics.sold_count / metrics.lead_count) * 100) : 0}%
        </CardDescription>
      </CardHeader>
      <CardContent>
